			...
			<table class="table table-bordered table-hover table-sm mb-0">
				<thead>
					<tr>
						...
						<th width="40%">Thông tin giao hàng</th>
				...
				<tbody>
					@foreach($donhang as $value)
						<tr>
							...
							<td>
								<span class="d-block">Điện thoại: <strong>{{ $value->dienthoaikhachhang }}</strong></span>
								<span class="d-block">Địa chỉ email giao: <strong>{{ $value->emailgiaohang }}</strong></span>
							...
							<td>{{ $value->tinhtrang == 0 ? 'Chưa xác nhận': 'Đã xác nhận'}}</td>
